...
trait Parse {
  ...
  implicit class ParserOps [A](p: Parser[A]) {
    ...
    def or(other: Parser[A]): Parser[A] = input => p(input) match {
      case Right(rest) => Right(rest)
      case Left(rest) => other(input)
    }
  }
  def numeric: Parser[Char] = input => input.toList.headOption match {
    case Some(value) if value.isDigit => Right((value, input.tail))
    case Some(value) => Left(ParseError(0, input, s"Expected digit, found '$value'"))
