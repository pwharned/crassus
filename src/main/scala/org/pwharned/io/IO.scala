...
object IO:
  ...
  /** Blocking conversion: when IO is run it blocks waiting for the given Future to complete */
  def fromFutureBlocking[A](fa: => Future[A], atMost: Duration = Duration.Inf)(using ec: ExecutionContext): IO[A] =
    Effect(() => Await.result(fa, atMost))

  /**
   * Non-blocking creation: registers callback on the Future now; when the IO is run it blocks waiting
   * on a small latch which the callback will fulfill. This makes creation non-blocking while still
   * providing simple semantics when unsafeRun is called.
   */
    ...
    def loop(): Any =
      current match
        case Pure(v) =>
          stack match
            case h :: tail =>
              current = try h(v) catch case e: Throwable => throw e
              stack = tail
              loop()
            case Nil =>
              v
        case Effect(thunk) =>
          val v = try thunk() catch case e: Throwable => throw e
          current = Pure(v)
          loop()
        case FlatMapped(src, f) =>
          src match
            case Pure(v) =>
              current = try f(v) catch case e: Throwable => throw e
              loop()
            case Effect(th) =>
              val v = try th() catch case e: Throwable => throw e
              current = try f(v) catch case e: Throwable => throw e
              loop()
            case FlatMapped(innerSrc, g) =>
              ...
              loop()
            case other =>
              val cont: Any => IO[Any] = (x: Any) => f(x.asInstanceOf).asInstanceOf[IO[Any]]
              stack = cont :: stack
  ...
  /**
   * Optimized run used by unsafeRunOptimized:
   * tries common fast shapes (Pure, Effect, FlatMapped(Pure/Effect/...)) with a limited depth,
   * falling back to run() if the structure is more complex or we exceed depth.
   */
  def unsafeRunOptimized[A](io0: IO[A], maxDepth: Int = 1000): A =
    @tailrec
    def fastLoop(current: IO[Any], stack: List[Any => IO[Any]], depth: Int): Any =
      if depth > maxDepth then
        ...
        current match
          case Pure(v) =>
            stack match
              case h :: tail =>
                val next = try h(v) catch case e: Throwable => throw e
          ...
          case FlatMapped(Pure(v), f) =>
            val next = try f(v) catch case e: Throwable => throw e
