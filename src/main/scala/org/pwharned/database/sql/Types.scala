...
transparent inline def mapToSqlTypes[A <: Tuple](
    mapper: DbTypeMapper
): List[String] =
...
def unwrappedTypeImpl[T: Type](using q: Quotes): Expr[String] = {
  ...
  def loop(tpe: TypeRepr): String = tpe.dealias match {
    ...
    case t if t =:= TypeRepr.of[java.util.UUID]      => "java.util.UUID"

    // Handle Option by unwrapping its inner type.
    case AppliedType(t, List(arg)) if t.typeSymbol.name == "Option" =>
      s"Option[${loop(arg)}]"
    case AppliedType(t, List(arg)) if t.typeSymbol.name == "List" =>
      s"List[${loop(arg)}]"
    ...
    case other =>
      report.warning(
        s"Potentially Unsupported field type: ${other.show} will be converted to String"
      )
      "String"
  }
