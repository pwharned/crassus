...
object Select:
  ...
  def selectImpl[T: Type](using q: scala.quoted.Quotes): scala.quoted.Expr[Select[T]] =
    import q.reflect.*
          ...
          case AppliedType(_, args) if args.nonEmpty =>
            args.head match
              case tref: TypeRef =>
                ...
                if asym != Symbol.noSymbol && asym.name.nonEmpty then Some(asym.name) else None
              case other => None
    ...
    val namesListOpt: Option[List[String]] =
      extractNameTuple(tpe).flatMap { namesTuple =>
        def tupleElems(tp: TypeRepr): List[TypeRepr] = tp match
          case AppliedType(tpSym, args) if tpSym.typeSymbol.fullName == "scala.Tuple" || tpSym.typeSymbol.fullName == "scala.EmptyTuple" =>
            args.flatMap(tupleElems) // defensive; usually scala.Tuple constructors vary by arity
    ...
    val sqlExpr = Expr(s"select ${namesListOpt.getOrElse(List("*")).mkString(",") } from  ${aliasOpt.getOrElse("<<no-alias-found>>")};")


    val emitted = '{
      val __selectInstance: org.pwharned.database.macros.Select[T] = new org.pwharned.database.macros.Select[T]:
        val select: String = $sqlExpr
      __selectInstance
    }
    report.info(s"select ${namesListOpt.getOrElse(List("*")).mkString(",") } from ${aliasOpt.getOrElse("<<no-alias-found>>")}")
    emitted
