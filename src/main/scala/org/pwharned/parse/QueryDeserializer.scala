...
object QueryDeserializer:
      ...
      def parser: String => Either[ParseError, (Vector[T], String)] =
        input =>
          ...
          if inner.isEmpty then Right((Vector.empty, ""))
            ...
            val init: Either[ParseError, (Vector[T], String)] = Right(
              (Vector.empty, "")
            )
      ...
      def parser: String => Either[ParseError, (List[T], String)] =
        input =>
          ...
          if inner.isEmpty then Right((Nil, ""))
            ...
            val tokens = inner.split(",").toList.map(_.trim)
            // foldLeft to accumulate (List[T], "") without needing a final reverse:
            val init: Either[ParseError, (List[T], String)] = Right(
              (List.empty, "")
            )
            val result = tokens.foldLeft(init) { (accE, tok) =>
              for {
  ...
  def optKeyValuePair[A](
    ...
    input =>
      ...
      if (input.trim.startsWith(key + "=")) {
        ...
        keyValuePair(key, valueParser)(input).map { case (v, rest) =>
          (Some(v), rest)
        }
      } else {
  ...
  inline def fieldParser[h](key: String): Parser[h] = {
    inline erasedValue[h] match {
      case _: Option[t] =>
        optKeyValuePair(key, summonInline[QueryFieldDeserializer[t]].parser)
          .asInstanceOf[Parser[h]]
      case _ =>
      ...
      case _: (h *: t) =>
        ...
        input =>
          headParser(input) match {
            case Left(err) => Left(err)
            case Right((hValue, r)) =>
              deriveParsers[t](tailNames)(r) match {
                case Left(err) => Left(err)
                ...
                case e: Exception =>
                  Left(
                    ParseError(
                      0,
                      s,
                      s"Error constructing instance: ${e.getMessage}"
                    )
                  )
          case Left(err) => Left(err)
        }
...
extension (s: String)
  inline def fromQuery[A <: Product]: Either[ParseError, A] =
    summonInline[QueryDeserializer[A]].deserialize(s)
