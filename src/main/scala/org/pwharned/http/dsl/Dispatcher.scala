...
object Dispatcher:
  ...
  inline def build(
      inline routes: (
          String,
          String,
          HttpRequestView => IO[HttpResponse[String]]
      )*
  ...
  private def buildImpl(
      routesExpr: Expr[
        Seq[(String, String, HttpRequestView => IO[HttpResponse[String]])]
      ]
  ...
  private def buildPathMatcher(
      routes: List[
        (String, List[String], Expr[Any], Type[?])
      ]
  )(using Quotes): Expr[(HttpRequestView, ByteBuffer, SocketChannel) => Unit] =
    import quotes.reflect.*
    ...
    val cases: List[Expr[
      (List[String], HttpRequestView, ByteBuffer, SocketChannel) => Boolean
    ]] =
            ...
            case '[e] =>
              '{
                (
                  ...
                  val respIO = $handlerExpr
                    .asInstanceOf[(HttpRequestView => IO[HttpResponse[String]])]
                    .apply(req)
