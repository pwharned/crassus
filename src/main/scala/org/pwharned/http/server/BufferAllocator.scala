...
/**
 * Type class for buffer allocation and pooling.
 * Critical for zero-allocation performance.
 */
...
object BufferAllocator:
  /**
   * Direct buffer pool - best for I/O operations.
   * Pooled to avoid allocation overhead.
   */
  def direct(size: Int, poolSize: Int = 100): BufferAllocator = new:
    ...
    def allocateRead(): ByteBuffer =
      Option(pool.poll()).map(_.clear()).getOrElse(ByteBuffer.allocateDirect(size))
