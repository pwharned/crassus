...
class HttpParser(maxHdr: Int = 8 * 1024) extends Parser[HttpRequestView] {
  ...
  private var pStart = -1; private var pEnd   = -1 // pEnd will now be the end of the *path* part
      ...
      state match {
        ...
        case 0 if b == SP && mEnd >= 0 && pEnd < 0 =>
          ...
          if (qMarkIndex == -1) {
            ...
            pEnd = requestTargetEnd
            qStart = requestTargetEnd + 1 // Mark query as empty (start after path)
            ...
            qEnd = requestTargetEnd // Query ends at the end of the request target
          }
  ...
  override def take(): Option[HttpRequestView] = {
    if state != 2 then None // Must be in state 2 (headers done, body offsets determined) to take.
    else
      buf.flip() // Prepare buffer for reading
      val view = new HttpRequestView(
        buf =buf.duplicate().limit(readBytes), // Pass a duplicate buffer limited to actual content
        methodOff= (mStart,   mEnd -  mStart),
        pathOff = (pStart,   pEnd -  pStart), // NEW: This is now just the path part
