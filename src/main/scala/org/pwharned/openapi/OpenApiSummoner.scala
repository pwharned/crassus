...
object OpenApiSummoner:
  ...
  /** 2) Deferred dispatch: build a String => Route function once, then call it
    * at runtime for each incoming path
    */
  inline def dispatchRoutePathFn(
      inline cfg: OpenApiConfig,
  ...
  private def dispatchRoutePathFnImpl(
    ...
    given fromExprServer: FromExpr[server] = new FromExpr[server] {
      override def unapply(x: Expr[server])(using Quotes): Option[server] =
        x match {
    ...
    given FromExpr[List[server]] = new FromExpr[List[server]] {
      override def unapply(
          x: Expr[List[server]]
      )(using Quotes): Option[List[server]] = x match {
        case '{ List(${ Varargs(elems) }*) } => {
          val elemFromExpr =
            elems.map(e => fromExprServer.unapply(e.asExprOf[server]))
          if elemFromExpr.forall(_.isDefined) then
            Some(elemFromExpr.flatten.toList)
          else None
        }
    ...
    val config = cfg match {
      case '{
            OpenApiConfig(
              version = ${ Expr(version: String) },
              title = ${ Expr(title: String) },
              description = ${ Expr(description: Option[String]) },
              servers = ${ Expr(servers: List[server]) }
            )
          } =>
        OpenApiConfig(version, title, description, servers)
    }
    ...
    def substituteSymbol(body: Term, from: Symbol, to: Symbol)(using
        Quotes
    ): Term = {
    ...
    val data: Seq[
      (String, List[String], Expr[HttpRequestView => IO[HttpResponse[String]]])
    ] = {
      RoutesExpr match
        case Varargs(es) =>
          es.map { routeExpr =>
                      ...
                      Apply(
                        ...
                        List(
                          Inlined(
                            Some(
                              Apply(
                                Apply(Ident(method), List(Ident("endpoint"))),
                                List(
                                  Typed(
                                    Repeated(
                                      List(Literal(StringConstant(path))),
                                      Inferred()
                                    ),
                                    Inferred()
                                  )
                ...
                val cleanHandler = handlerFunction match {
                  case fn @ Block(
                    ...
                    val substitutedBody = new TreeMap {
                      def transformTerm(tree: Term)(using Quotes): Term =
                        tree match {
                    ...
                    val lambdaExpr = '{ (x: HttpRequestView) =>
                      ${
                        substitutedBody(paramSym)
                          .asExprOf[IO[HttpResponse[String]]]
                      }
                    }
                    lambdaExpr
    ...
    val paths = data
      ...
      .map(x => {
        ...
        val delete =
          x._2.find(x => x._1 == "DELETE").map(x => x._2.mkString("/"))

        val getOperation =
          new operation("", s"get_$get", List.empty, None, Map.empty, None)
        val postOperation =
          new operation("", s"pst_$post", List.empty, None, Map.empty, None)
        val patchOperation =
          new operation("", s"patch_$patch", List.empty, None, Map.empty, None)
        val putOperation =
          new operation("", s"put_$put", List.empty, None, Map.empty, None)
        val deleteOperation = new operation(
          "",
          s"delete_$delete",
          List.empty,
          None,
        ...
        (
          x._1,
          pathItem(
            get = Some(getOperation),
            patch = Some(patchOperation),
            post = Some(postOperation),
            put = Some(putOperation),
            delete = Some(deleteOperation)
          )
        )
